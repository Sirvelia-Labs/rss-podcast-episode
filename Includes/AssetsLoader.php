...
class AssetsLoader
{
	...
	public function load_assets($handler)
	{
		foreach ($this->styles as $style) {
			if(is_null($style['condition']) || call_user_func($style['condition'], $handler)) wp_enqueue_style("rss-podcast-episode/{$style['name']}", $style['path'], $style['dependencies'], RSSPODCASTEPISODE_VERSION);
		}

		foreach ($this->scripts as $script) {
			if(is_null($script['condition']) || call_user_func($script['condition'], $handler)) {
				wp_enqueue_script("rss-podcast-episode/{$script['name']}", $script['path'], $script['dependencies'], RSSPODCASTEPISODE_VERSION);

				if($script['localize']) {
					wp_localize_script("rss-podcast-episode/{$script['name']}", $script['localize']['name'], $script['localize']['args']);
				}
			}
		}
	}
